...
while {PMC_killedNum < PMC_VictoryCondition} do
{
	waitUntil
	{
		...
		( (((count PMC_opfor_list < _MaxOPFOR) || (count PMC_blufor_list < _MaxBLUFOR)) && (count PMC_opfor_list + count PMC_blufor_list) < _PMC_maxunits) );
	};
	...
	if (count PMC_opfor_list < _MaxOPFOR) then
	{
		...
		_respawnpoint = [] call PMC_SelectOPFORStartPosit;
		_targetPoint = (getPosASL call PMC_Select_Target);
		[] call PMC_Create_NVA_Sniper;

		_respawnpoint = [] call PMC_SelectOPFORStartPosit;
		_targetPoint = (getPosASL call PMC_Select_Target);
		[] call PMC_Create_NVA_Recon;
	};
...
waitUntil
{
	...
	( (count PMC_opfor_list + count PMC_blufor_list) < _PMC_maxunits );
};

diag_log format["PMC_war.sqf finished with proper victory condition, time is now: %1, BLUFOR: %2, OPFOR: %3", time, count PMC_blufor_list, count PMC_opfor_list];
