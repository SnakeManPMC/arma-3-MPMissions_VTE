/********************************************************************************* 
   ...
   Example:
      res = ["AV8B2", false, false]execVM"mando_bombs_basic_mapclick.sqf"
...
{
   onMapSingleClick "res = [_pos, objNull, side player, 3, mando_sadarm_requested, mando_airsupport_type]execVM""mando_basic_airsupport.sqf""; onMapSingleClick '';true;";

   titleText[">>> Open the map and click over it for air support coordinates", "PLAIN DOWN"];
}
else
{
   onMapSingleClick "res = [_pos, objNull, side player, mando_sadarm_requested, mando_airsupport_type]execVM""mando_basic_bombrun.sqf"";  onMapSingleClick '';true;";
