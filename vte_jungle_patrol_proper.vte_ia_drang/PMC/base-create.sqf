...
private ["_A","_barrels1b1","_bx","_by","_c","_dshk1b1","_Eammo1b1","_fireb1","_fish","_grp","_mortar1b1","_mortar1b2","_PMC_OPFOR_Starts","_pmcfish","_poleb1","_RanDistance","_Rnd","_shedb1","_shedsmallb1","_tentb1","_tero","_truckb1","_mortar1b3","_mortar1b4","_unit1"];
...
while {_A < _c} do
{
	...
	_fish = [[_bx, _by], 0, 100, 10, 0, 20, 0] call BIS_fnc_findSafePos;

	if (PMC_debug) then
	{
		_tero = format ["basemk%1",_A];
		// changed these to LOCAL, maybe it breaks something? dunno
		_tero setMarkerPosLocal _fish;
	...
	_poleb1 = "VTE_flagcarrier_vc" createVehicle _fish;
	...
	// camp fire burning
	_fireb1 = "land_vte_vilfire1" createVehicle [(_fish select 0) + 5, (_fish select 1), 0];
	_fireb1 inFlame true;
	_fireb1 setDir random 360;

	// tents
	_tentb1 = "Land_VTE_UStent_small" createVehicle [(_fish select 0) + 10, (_fish select 1) + 10, 0];
	_tentb1 setDir random 360;

	// ammo boxes
	_Eammo1b1 = "VTE_AmmoBoxNVA" createVehicle [(_fish select 0), (_fish select 1) + 10, 0];
	_Eammo1b1 setDir random 360;
	_Eammo1b1 setVelocity [0,0,10];

	// sheds
	_shedb1 = "Land_VTE_vilbasket1" createVehicle [(_fish select 0) - 20, (_fish select 1), 0];
	_shedb1 setDir random 360;
	_shedsmallb1 = "Land_VTE_vilbasket1" createVehicle [(_fish select 0), (_fish select 1) - 10, 0];
	_shedsmallb1 setDir random 360;

	// barrels stuff
	_barrels1b1 = "Land_VTE_vilbasket1" createVehicle [(_fish select 0) - 20, (_fish select 1), 0];
...
*/
	_mortar1b1 = "VTE_82mm_mortar_nva" createVehicle [(_fish select 0) + 10, (_fish select 1) + 25, 0];
	_mortar1b1 setdir random 360;
	_mortar1b2 = "VTE_82mm_mortar_nva" createVehicle [(_fish select 0) - 10, (_fish select 1) - 25, 0];
	_mortar1b2 setdir random 360;
	_mortar1b3 = "VTE_82mm_mortar_nva" createVehicle [(_fish select 0) - 25, (_fish select 1) - 35, 0];
	_mortar1b3 setdir random 360;
	_mortar1b4 = "VTE_82mm_mortar_nva" createVehicle [(_fish select 0) + 25, (_fish select 1) + 35, 0];
	_mortar1b4 setdir random 360;

	// dshk machine guns NVA base camp
	_dshk1b1 = "VTE_dshkm" createVehicle [(_fish select 0) + 15, (_fish select 1) + 35, 0];
	_dshk1b1 setDir 0;

	// a truck
	_truckb1 = "VTE_nvatruck" createVehicle [(_fish select 0) - random 50 + random 100, (_fish select 1) - random 50 + random 100, 0];
...
*/
	_unit1 = _grp createUnit ["VTE_NVAsoldier", _fish, [], 1, "FORM"];
	_unit1 moveInGunner _mortar1b1;
	
	_unit1 = _grp createUnit ["VTE_NVAsoldier", _fish, [], 1, "FORM"];
	_unit1 moveInGunner _mortar1b2;
	
	_unit1 = _grp createUnit ["VTE_NVAsoldier", _fish, [], 1, "FORM"];
	_unit1 moveInGunner _dshk1b1;
	
	_unit1 = _grp createUnit ["VTE_NVAsoldier", _fish, [], 1, "FORM"];
	_unit1 moveInGunner _mortar1b3;
	
	_unit1 = _grp createUnit ["VTE_NVAsoldier", _fish, [], 1, "FORM"];
	_unit1 moveInGunner _mortar1b4;
